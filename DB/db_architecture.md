...
1. エンティティの抽出
   ...
   5. イベント系（「〇〇日」に当てはまるもの）から考え、その後リソース系（「〇〇名」にあてはまるもの）を挙げていくとスムーズ！
2. エンティティの定義
...
### 正規化の本質
- IDの導入は◎：重複・冗長性を排除する上で、IDという恣意的な属性追加は好ましくないとされている。しかし、IDの導入によってテーブル間の関係を疎結合に（洗い替え問題の回避）でき、データ構造の変更が非常に柔軟になる。 
- 業務の視点：単純に重複・冗長性を排除していくのではなく、業務において、ビジネスルールにおいてのDBの機能性を考えるべし。ER図を見ると一見重複があっても、それが必要不可欠な設計であるケースも多々ある。
...
- 重複したデータがあると、データ容量多くなる＆変更時に手間が大きいので、重複がないことが重要

### ID（アイデンティファイアの本質）
- インスタンス（＝レコード）を一位に特定する座標の役割
- エンティティ内におけるインスタンスのデータライフサイクル(DLC, i.e. create~delete)を表現
  - ※ 論理削除（＝DB上から消すのではなく、フラグをつけて削除されたとみなす）を行う場合は、アイデンティファイアはライフサイクルの表現の役割はもたない
- アイデンティファイアの再利用は避けるべき
...
### 論理設計と物理設計
- 論理設計（概念スキーマの設計）：データを管理するためのデータモデルの設計。**ERDの設計。**
- 物理設計（内部スキーマの設計）：DDLによる実装やストレージの構成などの設計
  - DDL: SQLでいうと、create文やdrop文
  - **インデックスの設計**
    - ディスクの配置など、ハードの性能をフルに引き出せるような状態にすること
    - インデックス：RDBの全件総なめを防ぐ。RDBMSのレコードは順序が保証されていないため、全件を見ていくしかない。しかし、インデックスがあれば、指定された順序で整列された状態から探すので、効率的に検索できる。
    - デメリット：整列するという役割上、データ更新のたびに並び替え処理が生まれ負荷がかかる

### SQLとは
- プログラミング言語ではなく、アクセス言語
- SELECT文１つの裏でも、非常に多くの内部処理（バッチ処理）が行われている
### SQLの３つの機能
...
- VARCHAR：可変長文字列。CHAR型と同様に最大文字数を指定するが、文字数は入力値の文字数がそのまま反映される。
- DATE：年月日

### 条件式
- 比較演算子 `a<>b` : aとbは等しくない
- LIKE : 文字列の部分一致検索。`%`を用いて、前方/中間/後方一致を検索。
- IN：ORで複数の条件式をつなげる時にスッキリかける
- BETWEEN：範囲検索（以上・以下しかできない）

### SQLクエリ
- SELECT -> FROM -> JOIN -> WHERE -> GROUP BY -> HAVING -> ORDER BYの順に書く
- JOIN
  - INNER JOIN（内部結合）

## ビュー
- よく使うSELECT文を保存して、使いまわすことができる機能
- ビューが参照されると保存されたSELECT文を実行して仮想のテーブルを作る

## サブクエリ
- ビュー定義のSELECT文をFROM句に書き込んだもの（使い捨てのビュー）
- 入れ子の階層は浅めがパフォーマンス的に良い
- 集約関数を組み合わせた分析を可能にする
### スカラサブクエリ
- 取得する値が単一の値（スカラ）になるサブクエリのこと
- １つの数値や文字列などを取得, not テーブル取得
- 集約関数がWHERE句に書けないからこそ、スカラサブクエリが必要

## 集合演算
- UNION: レコードの足し算

## なぜRDBMSが生まれたか
- 1970年代、プログラムの大量生産の必要性から、１つ１つのプログラムを効率的かつシンプルに記述しようとした
- その結果、１つのプロジェクトあたりのプログラム数が膨大なものとなり、ビジネスの状況の変化に応じたユーザーニーズに応えることが困難に
- 当時のハードウェアの性能の低さもあり、頭打ち状態に…
- そんな状況を打破すべく考案されたのが、「効率より効果」「無駄なプログラムは作らない」を主眼とするRDBMS！
