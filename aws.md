...
## Webレイヤの冗長化（ELB）
- ELBは稼働率を上げるためのAWSサービス

### 稼働率
- 稼働率：可用性の指標のひとつ
- 稼働率を高くする＝障害発生間隔↑＆平均復旧時間↓ 
- 冗長化 = 単一障害点（Single Point Of Failure）をなくす
- 稼働率を上げる方法
  - 要素単体の稼働率を高くする（←AWS利用時には考慮不要）
  - 要素を組み合わせてサービスの構成を冗長化
    - 冗長化構成
      - Active-Active: 冗長化した両方が利用可能
      - Active-Standby: 冗長化した片方は利用可能
        - Hot Standby: スタンバイ側は普段起動しすぐに利用可能
        - Warm Standby: スタンバイ側は普段起動しているが、利用するのに準備必要
        - Cold Standby: スタンバイ側は普段停止している
  - 負荷を適切なプロビジョニングで回避
    - スケールアップ
      - サーバーなどの個々の要素の**性能**を向上
    - スケールアウト
      - サーバーなどの個々の要素の**数**を増やす
      - ある程度の規模を超えそうであれば、アップよりアウトの方がコスパ良い

### サーバー構成のベストプラクティス
- パターン１：Web server * 1, DB server * 1
  - 1台でサーバースペックが足りなくなったらDBを別のサーバーに切り出す
- パターン２：Web server * 2, DB server * 1
  - Webサービスのユーザー数が増えると、Web serverへの負担大に。Webの冗長化と負荷分散が目的
- パターン３：Web server * 2, DB server * 2
  - Webの冗長化と負荷分散に加えて、DB serverをマスタースレーブ方式（AWSでは"マルチAZ方式"）にしてDBの冗長化

### ELB
- ロードバランサー：各サーバーにアクセスを振り分け負荷分散する装置
- ELBの特徴: 
  - 複数のEC2インスタンスに負荷分散。ヘルスチェックを行い、正常なインスタンスにのみ振り分ける
  - スケーラブル（ELB自体も自動でスケールアウト・イン）
  - AZをまたがる構成
...
### CloudWatch
- AWSサービスのメトリクス（リソースの状況）を監視
- メトリクスに対して閾値を登録し、その条件を満たしたら通知する（アラーム発生）
  - Amazon SNSという通知サービスを利用
  - Amazon SNS: Topicを作成・管理。Publisherがメッセージを送信し、Subscriberが通知を受信するための通信チャネルとして機能。
    - SNSによってPublisherとSubscriberを疎結合にすることができる

### 作業順序
1. CloudWatchのアラームを作成（同時にSNSのトピックを作成）
   1. 通知受信するメールアドレスで"サブスクリクションの確認"が必要
2. アラートを確認
   1. `yes > /dev/null &`を複数回EC2上で実行して負荷かけて、アラートが来ることを確認
      1. `yes` = 標準出力に永遠に`y`を出力する
      2. `/dev/null` = 画面に表示されない場所 -> 高速で実行される
      3. `&` = backgroundで実行
3. 後片付け
   1. `ps aux | grep yes`でyes出力プロセスのpidを確認して`kill -9 <pid>`（`-9`=強制終了）
...
- IAM
  ...
  - IAM自体の利用は無料
- 用語
  - policy: アクセス許可の定義。「どのAWSサービスの」「どのリソースに対して」「どんな操作を」「許可する（しない）」を定義
  - user: 個々のアカウントのユーザー
  - group: IAM userの集合
  - role: 一時的にアクセスを許可したアカウントを発行できる。EC2やLambdaなどのAWSリソースに権限を付与するために使用。
- ベストプラクティス
  - 個々人にIAM Userを作成（don't use root user!）
  - ユーザーをグループに所属させ、グループに権限を割り当てる
  - 権限は最小限に
  - EC2インスタンスから実行するアプリケーションには、ロールを使用
  - 定期的に不要な認証情報を削除する

### 作業順序
1. IAM ポリシーの作成
2. IAM グループ（w/ ポリシーアタッチ）とユーザーを作成
3. IAM Roleを作成して、EC2にアタッチ（S3FullAccessのRoleアタッチ前後で`aws s3 ls`の成否が変化）